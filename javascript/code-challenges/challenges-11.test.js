...
const count = (target, input) => {
  return input.reduce((acc, crrv) => {
    const arrCount = crrv.reduce((inneracc, innercrrv) => {
      if(innercrrv === target) {
        return inneracc + 1;
      }
      return inneracc
...
const totalSum = (input) => {
 return input.reduce((acc, crrv) => {
    return acc + crrv.reduce((inneracc, innercrrv) => {
 return inneracc + innercrrv;
    }, 0)
...
let findShortest = (data) => {
  return data.reduce((shortest, nextCharacter) => {
    return Number (shortest.height) < Number(nextCharacter.height) ? shortest : nextCharacter
